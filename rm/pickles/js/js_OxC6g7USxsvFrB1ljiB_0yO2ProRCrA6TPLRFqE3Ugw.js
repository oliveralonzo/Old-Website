...
/**
 * @file
 * Implement the detectLoginStatus function.
 */

(function ($) {
  var cookie = $.cookie('ARToken');

  // If no cookie, there is nothing to do here.
  if (!cookie) {
    return;
  }

  // Call API to fetch user data, if available.
  $.ajax({
    url: 'https://apps.allrecipes.com/v1/users/me',
    method: 'GET',
    headers: {
      'content-type': 'application/json',
      'authorization': ('Bearer ' + unescape(cookie))
    }
  }).done(function (data) {
    // User data is available; build user dropdown.
    initUserMenu(data);
  }).fail(function (data) {
    // User data is not available; log it and do nothing.
    console.log('User token not recognized.');
  });

  function initUserMenu (data) {
    var $li = insertNavItem(data);
    insertSubmenu($li);
  }

  // Insert user dropdown toggle link.
  function insertNavItem (data) {
    var user = {
      name: data.name,
      image: data.photo.urls[0].url
    }
    var profile_pic = document.createElement('a');
    profile_pic.href = 'https://allrecipes.com/cook/my/';
    profile_pic.className = 'user--photo';
    var profile_pic_image = document.createElement('img');
    profile_pic_image.src = user.image;
    profile_pic_image.alt = user.name + ' profile picture';
    profile_pic.appendChild(profile_pic_image);

    var dropdown_link = document.createElement('a');
    dropdown_link.href = '#usermenu';
    dropdown_link.className = 'showdrop';
    var dropdown_toggle = document.createElement('div');
    dropdown_toggle.className = 'login-state authenticated-user';
    var dropdown_toggle_name = document.createElement('span');
    dropdown_toggle_name.className = 'username';
    dropdown_toggle_name.id = 'offCanvasDisplayName';
    dropdown_toggle_name.appendChild(document.createTextNode(user.name));
    var dropdown_toggle_icon = document.createElement('span');
    dropdown_toggle_icon.className = 'icon--chevron-down';
    dropdown_toggle.appendChild(dropdown_toggle_name);
    dropdown_toggle.appendChild(dropdown_toggle_icon);
    dropdown_link.appendChild(dropdown_toggle);

    var $li = $('li.nav-profile');
    $li.empty();
    $li.append(profile_pic);
    $li.append(dropdown_link);

    return $li;
  }

  // Insert submenu after link.
  function insertSubmenu ($li) {
    var submenuHTML = document.getElementById('usermenuSubmenuTemplate').innerHTML;
    var container = document.createElement('div');
    container.className = 'nav-tab social profile-nav menudrop';
...
/**
 * @file
 * Polyfills EventTarget.addEventListener().
 *
 * @see
...
/**
 * @file
 * Initializes Adobe Dynamic Tag Management.
 */

(function () {
  var script = document.getElementById('meredith_atm_external_script');

  // Run ATM footer script after external script has loaded asynchronously.
...
/**
 * @file
 * Uses theme breakpoints for breakpoint change and faux mobile detection.
 */
...
(function ($, Drupal, debounce) {
  ...
  Drupal.breakpoints = Drupal.breakpoints || {};

  Drupal.breakpoints.current = null;

  var changeDetectionEnabled = false;

  /**
   * Detects whether or not the page is considered mobile based on window width.
   *
   * Because all pages are cached the same, regardless of the device visiting
   * them, Meredith sites have traditionally deemed a user as mobile if their
   * device is showing the smallest breakpoint.
   *
   * @param {string} [mobileBreakpointName]
   *   The name of the mobile breakpoint on the given site. Defaults to 'small'.
   *
   * @return {Boolean}
   *   Whether or not the current breakpoint matches mobileBreakpointName.
   */
  Drupal.breakpoints.isMobile = function (mobileBreakpointName) {
    setCurrentBreakpoint();
    mobileBreakpointName = mobileBreakpointName || 'small';
    return Drupal.breakpoints.current == mobileBreakpointName;
  };

  /**
   * Enables the reporting of when the page enters a different breakpoint.
   *
   * This function must be called before the page will report when it has
   * entered a different breakpoint. This exists so that sites that only need
   * mobile detection don't add an unnecessary resize listener to the page.
   *
   * @example
   * Drupal.breakpoints.enableChangeDetection();
   * $('body').on('breakpoint-change', function (event, currentBreakpoint) {
   *   // React to the breakpoint change.
   * });
   */
  Drupal.breakpoints.enableChangeDetection = function () {
    if (!changeDetectionEnabled) {
      $(window).on('resize.breakpoints', debounce(setCurrentBreakpoint, 250)).trigger('resize.breakpoints');
      changeDetectionEnabled = true;
    }
  };

  /**
   * Detects which breakpoint the page is currently being viewed at.
   */
  function setCurrentBreakpoint() {
    var currentBreakpoint = null;
    $.each(drupalSettings.breakpoints, function (name, breakpoint) {
      if (window.matchMedia(breakpoint.media_query).matches) {
        currentBreakpoint = name;
      }
    });

    if (Drupal.breakpoints.current != currentBreakpoint) {
      Drupal.breakpoints.current = currentBreakpoint;

      // @see Drupal.breakpoints.enableChangeDetection()
      if (changeDetectionEnabled) {
        $('body').trigger('breakpoint-change', currentBreakpoint);
      }
    }
  };
